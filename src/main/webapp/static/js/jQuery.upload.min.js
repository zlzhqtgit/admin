(function(a) {
	a.fn.upload = function(D) {
		...
		r.append(A.attr("data-num", B.num).attr("data-type", B.type)).append(
				d.attr("multiple", B.num > 1 ? "multiple" : false)).append(
				b.attr("name", B.inputname)).addClass(
				B.num > 1 ? "multiple" : "one").on("click", "li.add",
				function() {
					d.click()
		...
		if (B.value) {
			...
			for ( var x in p) {
				if (s < B.num && p[s]) {
					h = a("<li class='item success'></li>").append(g).append(
							typeof D == "function" ? E : "").data("url", p[s])
							.attr("data-filename", c(p[s])).css(
									"background-image", "url('" + p[s] + "')")
							.insertBefore(A);
					...
					var i = l(G[F], function(K, J, I) {
						...
						/*a.ajax({
							...
							success : function(M) {
								...
								if (M.code) {
									J.addClass("success").append(g).append(
											typeof D == "function" ? E : "")
											.data("url", M.msg)
								} else {
									J.addClass("error").attr("data-error",
											M.msg ? M.msg : "服务端返回数据异常")
								}
								q()
							},
							xhr : function() {
								...
								M.upload.addEventListener("progress", function(
									...
									if (N.lengthComputable) {
										var O = Math.round(N.loaded * 100
												/ N.total);
										J.children("svg.progress").css(
												"height",
												50 + (120 * O / 100) + "%");
		...
		function l(G, L) {
			...
			var F = G.size, J = G.name.toLowerCase().split(".").splice(-1)
					.join(), i = a(w);
			var I = B.type.split(",");
			if (I.indexOf(J) < 0) {
				var K = "不能上传." + J + "的文件!"
			...
			if (K) {
				i.append(k(J)).addClass("error").append(
						"<div class='filename'>" + G.name + "</div>").attr(
						"data-error", K).insertBefore(A);
				r.removeClass("empty");
				q();
				return false
			}
			...
			H.readAsDataURL(G);
			H.onload = function() {
				var P = c(G.name) + c(G.type) + c(G.size.toString())
						+ c(H.result);
		...
		function v(G) {
			var i = G.split(","), I = i[0].match(/:(.*?);/)[1], F = atob(i[1]), J = F.length, H = new Uint8Array(
					J);
			while (J--) {
				H[J] = F.charCodeAt(J)
			}
		...
		function c(F) {
			var J = 5381, I = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_-"
					.split(""), G = F.length - 1;
			if (typeof F == "string") {
				for (; G > -1; G--) {
		...
		function k(i) {
			...
			if ([ "zip", "rar", "gzip", "7-zip", "zipz", "rarr", "iso" ]
					.indexOf(i) >= 0) {
			...
			if ([ "avi", "wmv", "mpeg", "mp4", "mov", "mkv", "flv", "f4v",
					"m4v", "rmvb", "rm", "3gp", "dat", "ts", "mts", "vob" ]
					.indexOf(i) >= 0) {
